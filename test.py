...
if "history" in st.session_state and st.session_state.history:
    ...
    feedback = []
    if negative >= 3:
        feedback.append("⚠️ 강아지가 스트레스 신호를 자주 보내고 있어요. 산책이나 놀이 시간을 늘려주세요.")
    if any("낑낑거림" in h["행동"] for h in st.session_state.history):
        feedback.append("🩺 강아지가 낑낑거림을 자주 한다면 불안 또는 통증일 수 있으니 관찰이 필요합니다.")
    ...
    if positive >= 3 and negative == 0:
        feedback.append("🎉 강아지가 아주 행복한 하루를 보내고 있어요! 지금처럼만 돌봐주세요.")
    if not feedback:
